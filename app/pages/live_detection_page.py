...
class LiveDetectionPage(QWidget):
    ...
    def check_and_create_profile(self, camera_id, camera_source):
        """
        Check if quality profile exists for camera, create if not
        Only runs once per camera during initialization
        """
        if not self.quality_matcher:
            print(f"[PROFILE] Quality matcher not available")
            return False

        # Check if already processed
        if camera_id in self.profiles_checked:
            print(f"[PROFILE] Already checked for {camera_id}")
            return self.profiles_checked[camera_id]

        print(f"\n{'='*60}")
        print(f"[PROFILE] Checking quality profile for {camera_id}")
        print(f"{'='*60}")
        ...
        print(f"[PROFILE] No existing profile found, creating new profile...")

        try:
            # Open camera temporarily to capture a frame
            cap = cv2.VideoCapture(camera_source)

            if not cap.isOpened():
                print(f"[PROFILE ERROR] Failed to open camera for profile creation")
            ...
            # Capture frame for profile
            ret, frame = cap.read()
            cap.release()

            if not ret or frame is None:
                print(f"[PROFILE ERROR] Failed to capture frame")
                self.profiles_checked[camera_id] = False
                return False

            print(f"[PROFILE] Captured frame: {frame.shape}")

            # Extract quality profile
            profile = self.quality_matcher.extract_quality_profile(frame, camera_id)

            print(f"[PROFILE] ✓ Profile created and saved for {camera_id}")
            print(f"[PROFILE] Properties extracted:")
            ...
            print(f"[PROFILE ERROR] Failed to create profile: {e}")
            import traceback
            traceback.print_exc()
            self.profiles_checked[camera_id] = False
            print(f"{'='*60}\n")
