...
class ModelManager:
    """
    Singleton class to manage shared InsightFace model
    Optimized for multi-threaded camera workers
    """
    ...
    def _initialize_model(self):
        """
        Initialize InsightFace model with optimized settings
        Called only once across all camera workers
        """
        if ModelManager._model is None:
            ...
            try:
                ...
                ModelManager._model = FaceAnalysis(
                    name='buffalo_sc',  # Lightweight model for speed
                )

                # Optimized preparation
                # det_size=(640, 640) is good balance of speed and accuracy
                # Lower values like (320, 320) for faster processing
